...
var TemplateManager = {
    ...
    loadEvent : function(){
        ...
        Data.$generalContainer.on( "click" , ".sm-filter-date a", {dropdownClass : ".sm-filter-date" , filterType :"filterDate"},this.onClickFilter);
    ...
    applyFilterMix: function($dropdown, filterClass, filterType) {
        ...
        tmpVisible.not(".filterSource").not(".filterTag").not(".filterDate").removeClass("sm-hidden");
    ...
    getTagData: function(tag){
        if(TemplateManager.tags[tag] === undefined ){
            ...
            var tagObj = {name : tag , hash : hash};
            TemplateManager.tagsData.push(tagObj);
            return tagObj;
        }else{
            return { name : tag , hash : TemplateManager.tags[tag]};
        }
    },
